...
def _credit_note_payload(cn, inv: Invoice, org: Organization) -> dict:
    """
    Build the KRA eTIMS payload for a credit note — a refund receipt
    (rcptTyCd "R") referencing the original invoice.
    """
    vat = float(cn.vat_amount)
    subtotal = float(cn.subtotal)
    total = float(cn.grand_total)

    if vat == 0:
        taxble_b, taxble_e, tax_amt_b = 0.0, subtotal, 0.0
    else:
        taxble_b, taxble_e, tax_amt_b = subtotal, 0.0, vat

    now_dt = datetime.now(timezone.utc)
    issue_dt = (cn.created_at or now_dt).strftime("%Y%m%d%H%M%S")

    items_payload = []
    for i, item in enumerate(cn.items, start=1):
        items_payload.append({
            "itemSeq": i,
            "itemCd": f"ITEM{str(i).zfill(4)}",
    ...
    return {
        ...
        "orgInvcNo": 1,
        "cfmDt": issue_dt,
        "salesDt": (cn.created_at or now_dt).strftime("%Y%m%d"),
        "stockRlsDt": None,
        "rcptTyCd": "R",  # refund / credit note
        "pmtTyCd": _payment_code(inv.payment_method),
        "salesSttsCd": "02",
        "salesTyCd": "N",
        "cancelReqDt": None,
        "rfdDt": now_dt.strftime("%Y%m%d%H%M%S"),
        "rfdRsnCd": "06",  # other
        "totItemCnt": len(cn.items),
        "taxblAmtA": 0,
        "taxblAmtB": taxble_b,
        "taxblAmtC": 0,
        "taxblAmtD": 0,
        ...
        "totAmt": total,
        "prchrAcptcYn": "N",
        "remark": f"Credit note {cn.id} against invoice {inv.id}"
                  + (f" (CU {inv.etims_cu_invoice_no})" if inv.etims_cu_invoice_no else ""),
        "regrId": org.kra_pin or "SYSTEM",
        "regrNm": org.name or "System",
        "modrId": org.kra_pin or "SYSTEM",
        "modrNm": org.name or "System",
        "receipt": {
            ...
            "trdeNm": inv.client.name if inv.client else "",
            "adrs": inv.client.address if inv.client else "",
            "topMsg": f"Credit note for {inv.id}",
            "btmMsg": "Thank you for your business.",
            "prchrAcptcYn": "N",
        },
        "itemList": items_payload,
...
async def submit_credit_note_to_kra(db: AsyncSession, credit_note, org: Organization):
    """
    Submit a credit note to KRA eTIMS as a refund receipt and store the
    control unit data on the credit note. Raises ValueError on KRA error.
    """
    result = await db.execute(
        select(Invoice)
    ...
    if credit_note.etims_cu_invoice_no:
        raise ValueError("Credit note already submitted to eTIMS")

    payload = _credit_note_payload(credit_note, inv, org)
    url = f"{_base_url(org)}/TrnsSalesOsdc"
    ...
    if body.get("resultCd", "") != "000":
        raise ValueError(f"KRA rejected credit note: {body.get('resultMsg', 'Unknown error')}")

    data = body.get("data", {})
    credit_note.etims_cu_invoice_no = str(data.get("rcptNo", ""))
    credit_note.etims_rcpt_sign = data.get("rcptSign", "")
    credit_note.etims_int_data = data.get("intrlData", "")
    credit_note.etims_submitted_at = datetime.now(timezone.utc)
